...
export const GETTRANSD = `${PRE_FIX}GETTRANSD`;
export const POSTRANS = `${PRE_FIX}POSTRANS`;
...
//创建文章
export const postTrans = createAction(POSTART, params => {
	return http
		.post({
			url: "ex_notice",
			params
...
//修改文章
export const putTrans = createAction(POSTRANS, data => {
	return http
		.put({
			url: "ex_notice/" + data.id,
			params: data.data
		})
		.then(res => {
			return res;
...
//获取详情
export const getArticleDetail = createAction(GETTRANSD, data => {
	return http
		.get({
			url: "ex_notice/" + data.id,
			params: data.params

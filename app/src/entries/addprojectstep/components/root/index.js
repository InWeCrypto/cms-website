...
export default class Root extends PureComponent {
    ...
    componentDidMount() {
        ...
        if (query.c_id) {
            ...
            //判断是否是ico
            if (query.model == "ico") {
                this.setState({isIco: true});
            } else {
                this.setState({type: 1, onlyType: true});
            }
        }
        if (query.model == "ico") {
            this.setState({isIco: true});
        ...
        const that = this;
        if (query.lng == "en") {
            this.setState({lngText: "英文"});
        } else if (query.lng == "cn" || query.lng == "zh") {
            this.setState({lngText: "中文"});
        }
        //获取阿里云 key
        this
            .props
            ...
            .then(res => {
                if (res.code === 4000) {
                    ...
                    this.uploaderMedia("mediaImg", function (img) {
                        ...
                        that.setState({socialParent, mediaParentImg: img});
                    });
                    this.uploaderMedia("mediaQrImg", function (img) {
                        let socialParent = that.state.socialParent;
                        socialParent.qr_img = img;
                        that.setState({socialParent, mediaParentQrImg: img});
                    });
                }
            });
    }
    ...
    putMode() {
        ...
        this
            ...
            .getIndusty(data)
            .then(res => {
                this.setState({industy: res.data[0].name})
            })
    }
    ...
    firstEndAndNext() {
        ...
        if (type && name && long_name && img && website && unit && token_holder && desc) {
            ...
            if (isPutMode) {
                ...
                this
                    ...
                    .then(res => {
                        let query = getRouteQuery(this);
                        let queryStr = "lng=" + query.lng + "&c_id=" + c_id + "&putMode=yes";
            ...
            Modal.warning({title: "提示", content: "请将上面信息填写完整"});
        }
    }
    SecondPrevStep() {
        this.setState({
    ...
    SecondNextStep() {
        ...
        function isAllDone(isJump) {
            if (((browserFlag || browserObj.params.length == 0) && (walletFlag || walletObj.params.length == 0) && (socialFlag || socialObj.params.length == 0)) || isJump) {
                that.setState({
                    step: 3,
                    progressArr: [
                        {
        ...
        //都为空，直接跳转至下一步
        if (browserObj.params.length == 0 && walletObj.params.length == 0 && socialObj.params.length == 0) {
            isAllDone(true);
        }
    }
    public() {
    ...
    getIndusty(val) {
        this.setState({industy: val});
    }

    getDropdownKey(key) {
        //var type = parseInt(key) + 1;
        this.setState({type: key, chargeType: key});
    }
    //上传图片
    uploader(data) {
        const option = data;
    ...
    getHolder(val) {
        this.setState({token_holder: val});
    }
    addBrowserChild() {
        ...
        this.setState({browserChild: arr});
    }
    ...
    getValNameOrUrlBrowserParent(val, idx, isName) {
        ...
        this.setState({browserParent: browserParent});
    }
    addWalletChild() {
    ...
    getValNameOrUrlWalletParent(val, idx, isName) {
        ...
        this.setState({walletParent: walletParent});
    }
    addSocialChild() {
        let that = this;
    ...
    getValNameOrUrlSocialParent(val, idx, isName) {
        ...
        this.setState({socialParent: socialParent});
    }
    getIcoPrice(val) {
        this.setState({ico_price: val});
    }
    getIcoDesc(val) {
                        ...
                        <div className="scrollBox holdBottom-3">
                            ...
                            <div className="inputBox mgt-3">
                                <Input
                                    ...
                                    .bind(this)}
                                    name="项目标签"
                                    placeholder="项目标签如：区块链"
                                    width="6.32rem"/>
                            </div>
                            {!onlyType && (
                                <div className="logo mgt-3">
                                    <Formtext name="项目类型" content={typeContent}/>
                                </div>
                            )}
                    ...
                    <div className="formBox1 eleScroll">
                        <div className="scrollBox holdBottom-3">
                            <div className="inputBox net">
                                ...
                                {browserChild && (
                                    <div className="in-child">
                                        {browserChild && browserChild.map((val, idx) => {
                                            return (
                                                <div
                                                    className={val.isHide
                                                    ? "netbox-c1 Hide"
                                                    : "netbox-c1"}
                                                    key={idx}>
                                                    <Input2
                            ...
                            <div className="inputBox wallet">
                                ...
                                {walletChild && (
                                    <div className="in-child">
                                        {walletChild && walletChild.map((val, idx) => {
                                            return (
                                                <div
                                                    className={val.isHide
                                                    ? "netbox-c1 Hide"
                                ...
                                <div className="in-parent">
                                    ...
                                    <div className="mediaUploadimg ui">
                                        ...
                                        <div className="iconUpload">
                                            <div className="icon" id="mediaImg"/>
                                            <img className="img" src={mediaParentImg} alt=""/>
                                        </div>
                                        媒体二维码上传,如微信二维码
                                        <div className="iconUpload">
                                            <div className="icon" id="mediaQrImg"/>
                                            <img className="img" src={mediaParentQrImg} alt=""/>
                                        </div>
                                    </div>
                                </div>
                                {socialChild && (
                                    <div className="in-child">
                                        {socialChild && socialChild.map((val, idx) => {
                                            console.log(val);
                                            return (
                                                <div className="socialMar" key={idx}>
                                                    <div
                                                        className={val.isHide
                                                        ? "netbox-c1 Hide"
                                                        : "netbox-c1"}
                                                    ...
                                                    <div className="mediaUploadimg ui">
                                                        媒体图标上传
                                                        <div className="iconUpload">
                                                            <div className="icon" id={"mediaImg" + idx}/>
                                                            <img
                                                                className="img"
                                                                src={val.img
                                                                ? val.img
                                                                : upload}
                                                                keyy={stringify(val.img)}
                                                                alt=""/>
                                                        </div>
                                                        媒体二维码上传,如微信二维码
                                                        <div className="iconUpload">
                                                            <div className="icon" id={"mediaQrImg" + idx}/>
                                                            <img
                                                                className="img"
                                                                src={val.qr_img
                                                                ? val.qr_img

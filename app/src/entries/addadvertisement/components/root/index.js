...
export default class Root extends PureComponent {
    ...
    componentDidMount() {
        ...
        this.setState({linkLang: q.lng, lang: q.lng});
        if (q.id) {
            this
                .props
                .getAdverDetail({id: q.id})
                .then(res => {
                    if (res.code === 4000) {
                        const {
                            type,
    ...
    upLoadImg(option) {
        ...
        uploader.on("beforeFileQueued", res => {
            var r = res;
            var name = parseInt(Math.random() * 10000000) + new Date().getTime();
        ...
        uploader.on("uploadSuccess", res => {
            if (res.id) {
                ...
                this.setState({img: imgAdd, imgadd: imgAdd});
            }
        });
    }
    changeType(type) {

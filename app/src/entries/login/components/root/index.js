...
export default class Root extends PureComponent {
    ...
    sendCode() {
        ...
        if (!phoneNum) {
            Modal.warning({title: "提示", content: "手机号不能为空"});
            return;
        }
        if (pattern.test(phoneNum)) {
            ...
            this
                ...
                .getCode(params)
                .then(res => {
                    if (res.code === 4000) {} else {
                        Modal.warning({title: "提示", content: res.msg});
                    }
                });
        } else {
            //alert("请输入正确的手机号");
            Modal.warning({title: "提示", content: "请输入正确格式到手机号"});
        }
    }
    getCode(e) {
    ...
    loginInClick() {
        ...
        let phoneNum = this.state.phoneNum;
        if (!code) {
            Modal.warning({title: "提示", content: "请输入验证码"});
            return;
        }
        let pattern = /^((1[3,5,8][0-9])|(14[5,7])|(17[0,6,7,8])|(19[7]))\d{8}$/;
        if (!phoneNum) {
